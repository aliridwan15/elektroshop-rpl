...
// --- Mengambil pesanan berstatus 'dikirim' dari Database ---
$sentOrders = [];
$stmt = $conn->prepare("
    SELECT
    ...
    JOIN
        produk prod ON td.id_produk = prod.id -- KOREKSI: Ini adalah join key yang benar
    WHERE
        p.id_user = ? AND p.status_pengiriman = 'dikirim' -- Memfilter berdasarkan status 'dikirim'
    ORDER BY
...
while ($row = $result->fetch_assoc()) {
    ...
    if (!isset($sentOrders[$pengirimanId])) {
        $sentOrders[$pengirimanId] = [
...
<style>
    ...
    /* Sidebar specific styles - Updated to match pending.php's .my-sidebar-order */
    .my-sidebar-order { /* Changed from .sidebar-order */
        ...
        min-width: 200px; /* Added from pending.php */
    }
...
<div class="container-fluid" style="padding: 40px 20px; background-color: #ffe3f1;">
    <div class="row order-row">
        <div class="col-md-3 mb-4">
            <div class="my-sidebar-order"> <h5 class="mb-4"><i class="fas fa-stream text-danger"></i> My Orders</h5>
        ...
        <div class="col-md-9">
            <h2>My Sent Orders</h2>
            <?php if (empty($sentOrders)): ?>
                <div class="alert alert-info text-center mt-3" role="alert">
                    Tidak ada pesanan yang sedang dalam status 'dikirim' saat ini.
                </div>
            <?php else: ?>
